...
export function Navigation() {
                    ...
                    >
                      <Link href={item.href}>
                        <a className="flex items-center gap-6 group">
                          ...
                          >
                            <item.icon
                              className={`w-6 h-6 ${isActive ? "text-[#c10157]" : "text-[#f2f8fc]/60"}`}
                            />
                          </div>
  ...
  return (
    <nav className="fixed top-6 left-1/2 -translate-x-1/2 z-[100] w-[90%] max-w-2xl">
      <div className="bg-transparent backdrop-blur-xl border border-white/10 rounded-full px-6 py-3 flex items-center justify-between shadow-2xl">
        {NAV_ITEMS.map((item) => {
          const isActive = location === item.href;
          return (
            <Link key={item.href} href={item.href}>
              <a className="relative group px-3 py-2 flex flex-col items-center gap-1 rounded-full hover:shadow-[0_0_20px_hsl(var(--primary)/0.4)] transition-all">
                <item.icon
                  className={`w-5 h-5 transition-colors duration-300 ${
                    isActive
                      ? "text-white"
                      : "text-[#f2f8fc]/60 group-hover:text-[#f2f8fc]"
                  }`}
                />
                <span
                  className={`text-[10px] uppercase tracking-widest transition-colors duration-300 ${
                    isActive
                      ? "text-white"
                      : "text-[#f2f8fc]/40 group-hover:text-[#f2f8fc]/80"
                  }`}
                >
                  {item.label}
                </span>
                {isActive && (
                  <motion.div
                    layoutId="nav-active"
                    className="absolute inset-0 border border-[#c10157] shadow-[0_0_10px_#fff,0_0_20px_#c10157,0_0_40px_#c10157,0_0_80px_#c10157] rounded-full -z-10 brightness-110"
                    style={{ backgroundColor: "#800030" }}
